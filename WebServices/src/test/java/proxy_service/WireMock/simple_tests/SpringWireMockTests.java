...
package proxy_service.WireMock.simple_tests;
...
public class SpringWireMockTests {
	...
	public void test1() throws ClientProtocolException, IOException, InterruptedException
	{
		try (CloseableHttpClient httpClient = HttpClients.createDefault()) {
			HttpGet request = new HttpGet("http://".concat(mockServiceHost).concat(":").concat(String.valueOf(mockServicePort)).concat("/context"));
			HttpResponse httpResponse = httpClient.execute(request);

			System.out.println(httpResponse);

			// Using AssertJ
			Assertions.assertThat(httpResponse.getStatusLine().getStatusCode()).as("200 OK expected here").isEqualTo(200);
